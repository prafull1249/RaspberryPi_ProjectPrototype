...
class Slave(object):
    ...
    def __init__(self, slave_id, unsigned=True, memory=None):
        ...
        self._fn_code_map = {
            ...
            READ_DISCRETE_INPUTS: self._read_discrete_inputs,
    ...
    def _read_discrete_inputs(self, request_pdu):
        ...
        return self._read_digital(DISCRETE_INPUTS, request_pdu)

    def _read_registers(self, block_type, request_pdu):
        """read the value of holding and input registers"""
        (starting_address, quantity_of_x) = struct.unpack(">HH", request_pdu[1:5])
        LOGGER.error("in read_registers00000 ")
        if (quantity_of_x <= 0) or (quantity_of_x > 125):
            # maximum allowed size is 125 registers in one reading
            LOGGER.error("quantity_of_x is %d", quantity_of_x)
            raise ModbusError(ILLEGAL_DATA_VALUE)
        LOGGER.error("read _registers modbus Error after %s " %starting_address)
        LOGGER.error("quantityt of x %s " %quantity_of_x)
        ...
        # implement a temporary random valuese range
        if(starting_address <= 40082 and starting_address >= 40077):
            value_type = "Voltage"
        elif(starting_address <= 40075 and starting_address >= 40072):
            value_type = "Current"
        elif(starting_address == 40084):
            value_type = "Power"
        else:
            value_type = "Frequency"

        valuesDir = type_registers[value_type]

        for i in range(0,quantity_of_x):
            values[i] = (random.uniform(valuesDir["range1"],valuesDir["range2"]))
        # write the response header
    ...
    def _read_input_registers(self, request_pdu):
        ...
        return self._read_registers(ANALOG_INPUTS, request_pdu)
    ...
    def _write_multiple_registers(self, request_pdu):
        ...
        # look for the block corresponding to the request
        block, offset = self._get_block_and_offset(HOLDING_REGISTERS, starting_address, quantity_of_x)
    ...
    def _write_multiple_coils(self, request_pdu):
        ...
        # look for the block corresponding to the request
        block, offset = self._get_block_and_offset(COILS, starting_address, quantity_of_x)
    ...
    def _write_single_register(self, request_pdu):
        ...
        block, offset = self._get_block_and_offset(HOLDING_REGISTERS, data_address, 1)
        block[offset] = value
    ...
    def handle_request(self, request_pdu, broadcast=False ):
        ...
        with self._data_lock:
            try:
                ...
                LOGGER.error("getting response now")
                # execute the corresponding function
                LOGGER.error(function_code)
    ...
    def _get_block(self, block_name):
        ...
        LOGGER.error("getting the block ")
...
class inverter(Slave):
    ...
    """
    def __init__(self, manufacturer = "Sunnyboy", version = "Something", model = "Sunnyboy GT15"):
        """
        Contructor: defines the address range and creates the array of values
        """

        self.holding =registers_holding
        self.init_holding_registers()
        self.serialNo = self._id
    ...
    def init_holding_registers(self):
        self.add_block('0',HOLDING_REGISTERS,40069,40110 - 40069)
...
class Databank(object):
    ...
    def add_inverter(self, slave_id, unsigned=True, memory=None,manufacturer = "Sunnyboy", version = "Something", model = "Sunnyboy GT15"):
        """Add a new inverter  with the given id"""
        with self._lock:
            if (slave_id <= 0) or (slave_id > 255):
                raise Exception("Invalid slave id {0}".format(slave_id))
            if slave_id not in self._slaves:
                self._slaves[slave_id] = inverter(slave_id, unsigned, memory)
                return self._slaves[slave_id]
            else:
                raise DuplicatedKeyError("Slave {0} already exists".format(slave_id))
    ...
    def get_slave(self, slave_id):
        ...
        with self._lock:
            if slave_id in self._slaves:
                ...
                LOGGER.error("something wrong with slave id")
    ...
    def handle_request(self, query, request):
        ...
        try:
            ...
            LOGGER.error("slaveid and request pdu is : " + str(slave_id) + " ")
            ...
            LOGGER.error("handle request inside : " + str(excpt))
...
class Server(object):
    ...
    def add_inverter(self, slave_id, unsigned=True, memory=None):
        """add slave to the server"""
        return self._databank.add_slave(slave_id, unsigned, memory)
